/** Static brand files (mixed extensions) served from /images/logos */
 ...
 * - Full http(s) URLs and absolute paths are left as-is.
 * - Bare filenames (any extension) resolve under /images/logos/.
 * If still empty, uses website hostname → /images/providers/ slug map, then host aliases.
 */
export function resolveProviderLogoUrl(imageField: string, website: string): string {
  ...
  return PROVIDER_SCREENSHOT_BY_SLUG[slug] ?? ''
}
